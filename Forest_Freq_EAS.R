...
writeLines(samples_with_CNV3mb, "samples_with_CNV3mb_EAS.txt")
...
# ### read cnv data
# snv.osc <- snv[snv$Ethnicity == "EAS", ]#[snv$OSC1_OSC2 == "OSC1", ]
...
for(freq_range in freq){
  ...
  missing.feats <- feats[!feats %in% names(dt.matrix)]
  if(length(missing.feats) > 0)
    dt.matrix[, missing.feats] <- 0
  for(i in 2:65){
...
write.table(dt.out, "../results/EAS/freq_forest_plot_cnvs.tsv", sep="\t", row.names=F, quote=F, col.names=T)
...
ggplot(dt.out, aes(x = freq, y = coefficient, color = plot_global)) + geom_hline(yintercept = 0, lty = 2, lwd = .5) +
  geom_point(aes(fill = global), color = "black", position = position_dodge(width = .5), shape = 21) +
  geom_errorbar(aes(ymin = coefficient.low, ymax = coefficient.up), position = position_dodge(width = .5), width = .2) + 
  coord_flip(ylim = c(-2, 2)) + facet_wrap(measure~.) + theme_bw() + theme(legend.position = "top") +
...
ggsave("../results/EAS/freq_forest_plot_cnvs.png", width = 7, height = 7)
