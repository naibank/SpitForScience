...
gene.in <- gene.in[which(gene.in$type_of_gene == "protein-coding" & gene.in$chr == "chrX"), ]
...
# snv.osc <- snv.osc[snv.osc$chr %in% "chrX", ]

# cnv[intersect(grep("PNPLA4", cnv$Gene_Exons), which(cnv$max_freq < 0.5 & cnv$cnvFlag == "Loss")), ]
# snv.osc[which(snv.osc$gene_symbol == "MAOB" & snv.osc$MSN == "DMSN" & snv.osc$freq_max < 0.5), ]


sampleinfo <- sampleinfo[!sampleinfo$TAG_id %in% c(outliers, samples_with_CNV3mb), ]
sampleinfo <- sampleinfo[which(sampleinfo$gender_genetic == "Male"), ]
...
xcount <- c()
for(igs in 1:length(gsOSC)){
  tmp.genes <- unique(genes$enzid)
  print(sprintf("%s %s", names(gsOSC)[igs], sum(gsOSC[[igs]] %in% tmp.genes)))
  xcount <- c(xcount, sum(gsOSC[[igs]] %in% tmp.genes))
}

gsOSC.subset <- gsOSC[which(xcount > 100)]
global.feats <- c("gene_count_gain", "gene_count_loss")
...
for(freq_range in freq){
  ...
  for(measure in c("swan_mf_tscores", "swan_ia_mf_tscores", "swan_hi_mf_tscores", 
    ...
    gs.out <- data.frame()
    for(gs in 1:length(gsOSC.subset)){
      feats <- paste(names(gsOSC.subset)[gs], c("gain", "loss"), sep="_") #, "LOF", "DMSN"), sep="_")
      
      ref <- lm(sprintf("%s ~ %s", measure, paste(c(covariates, global.feats), collapse = " + ")), th.matrix)
      add <- lm(sprintf("%s ~ %s + %s", measure, paste(c(covariates, global.feats), collapse = " + "), paste(feats, collapse = " + ")), th.matrix)
      ano <- anova(add, ref, test = "Chisq")
      conf <- confint(add)
      count <- sum(th.matrix[, feats] > 0)
      
      gs.test.out <- data.frame(measure, "freq" = paste(freq_range, collapse = "-"),
                                    "p" = ano$`Pr(>Chi)`[2], "coefficient" =  add$coefficients[feats],
                                    "coefficient.low" =  conf[feats, 1], "coefficient.up" =  conf[feats, 2],
                                "geneset" = names(gsOSC.subset)[gs],
                                "type" = gsub(sprintf("%s_", names(gsOSC.subset)[gs]), "",  feats),
                                "sample_count" = count)
      
      gs.out <- rbind(gs.out, gs.test.out)
    }
    gs.out <- gs.out[gs.out$sample_count >= 3, ]
    
    uniq.p <- unique(gs.out[, c("geneset", "p")])
    pos <- grep("Synapse|FMR1|Neurof|Bspan|PhMm_Ne", uniq.p$geneset)
    uniq.p$set <- "positive" 
    uniq.p$set[-pos] <- "control"
    uniq.p$BHFDR <- NA
    uniq.p$BHFDR[pos] <- p.adjust(uniq.p$p[pos], method = "BH")
    uniq.p$BHFDR[-pos] <- p.adjust(uniq.p$p[-pos], method = "BH")
    
    gs.out <- merge(gs.out, uniq.p[, -2], by = "geneset", all = T)
    
    dt.out <- rbind(dt.out, gs.out)
  }
  
  dir.out <- sprintf("../results/EURX/%s/%s", paste(freq_range, collapse = "-"), Sys.Date())
  
  if(!dir.exists(dir.out)){
    dir.create(dir.out, recursive = T)
  }
  
  write.table(dt.out, sprintf("%s/geneset_burden_cnv.tsv", dir.out), sep="\t", row.names=F, quote=F, col.names=T)
...
for(freq_range in freq){
  ...
  dir.in <- sprintf("../results/EURX/%s/%s", paste(freq_range, collapse = "-"), "2022-04-13")
  ...
  for(measure in unique(dt.out$measure)){
    ggplot(dt.out[dt.out$measure == measure, ], aes(x = geneset, y = coefficient, fill = BHFDR < 0.20 & p < 0.05)) +
      geom_bar(stat = "identity", color = "black", width = .5) + theme_bw() +
  ...
  dt.out$plot_type <- factor(dt.out$plot_type, levels = c("Deletions", "Duplications"))
  ...
  cowplot::plot_grid(m, s, nrow = 1, rel_widths = c(nrow(main)/3, nrow(sub)/3))
  ggsave(sprintf("%s/combined_geneset_cnv.png", dir.in), width = 10, height = 10)
}
