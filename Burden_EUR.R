...
freq <- list(c(0, 0.5), c(0.5,1), c(1,5))
load("../../gsOSC.RData")
...
global.feats <- c("gene_count_gain", "gene_count_loss") #, "Total_LOF", "Total_DMSN")
...
for(freq_range in freq){
  ...
  for(measure in c("swan_mf_tscores", "swan_ia_mf_tscores", "swan_hi_mf_tscores", 
    ...
    ref.global <- lm(sprintf("%s ~ %s", measure, paste(c(covariates), collapse = " + ")), th.matrix)
    add.global <- lm(sprintf("%s ~ %s", measure, paste(c(covariates, global.feats), collapse = " + ")), th.matrix)
    conf <- confint(add.global)
    sm <- summary(add.global)
    global.out <- rbind(global.out, data.frame(measure, "freq" = paste(freq_range, collapse = "-"),
                              "p" = sm$coefficients[global.feats, "Pr(>|t|)"], "coefficient" =  add.global$coefficients[global.feats],
                              "coefficient.low" =  conf[global.feats, 1], "coefficient.up" =  conf[global.feats, 2],
                              "feat" = global.feats))
    
    gs.out <- data.frame()
    for(gs in 1:length(gsOSC)){
      feats <- paste(names(gsOSC)[gs], c("gain", "loss"), sep="_") #, "LOF", "DMSN"), sep="_")
      ...
      add.nocorrection <- lm(sprintf("%s ~ %s + %s", measure, paste(covariates, collapse = " + "), paste(feats, collapse = " + ")), th.matrix)
      
      ano <- anova(add, ref, test = "Chisq")
      conf <- confint(add.nocorrection)
      count_gain <- sum(th.matrix[, feats[1]] > 0)
      count_loss <- sum(th.matrix[, feats[2]] > 0)
      gs.test.out <- data.frame(measure, "freq" = paste(freq_range, collapse = "-"),
                                    "p" = ano$`Pr(>Chi)`[2], "coefficient" =  add.nocorrection$coefficients[feats],
                                    "coefficient.low" =  conf[feats, 1], "coefficient.up" =  conf[feats, 2],
                                "geneset" = names(gsOSC)[gs],
                                "type" = gsub(sprintf("%s_", names(gsOSC)[gs]), "", feats),
                                "sample_count" = count_gain+count_loss, count_gain, count_loss)
      
      gs.out <- rbind(gs.out, gs.test.out)
    }
    
    gs.out <- gs.out[gs.out$count_gain >= 3 | gs.out$count_loss >= 3, ]
    uniq.p <- unique(gs.out[, c("geneset", "p")])
    pos <- grep("Synapse|FMR1|PhHs|Neurof|Bspan|PhMm_Ne", uniq.p$geneset)
    uniq.p$set <- "positive" 
  ...
  write.table(global.out, sprintf("%s/global_burden_cnv.tsv", dir.out), sep="\t", row.names=F, quote=F, col.names=T)
